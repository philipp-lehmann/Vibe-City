...
import { state, makeTile, inBounds, setTool, togglePause, rotateView,
         pushNotice, requestFlash, dragTiles } from './state.js';
import { view, screenToIso } from './renderer.js';
...
function placeTool(gx,gy){
  ...
  if(tool.id==='bull'){
    if(t.type!==T.GRASS){ if(spend(1)){ state.grid[gy][gx]=makeTile(T.GRASS); } }
    return;
  }
  ...
  if(spend(tool.cost)){ state.grid[gy][gx]=makeTile(tool.tile); lastPaint=key; }
}
...
export function initInput(){
  ...
  // scroll wheel zoom (2 levels) — ui syncs the button label
  view.addEventListener('wheel',e=>{
    e.preventDefault();
    state.zoom = e.deltaY<0 ? 2 : 1;
  },{passive:false});
